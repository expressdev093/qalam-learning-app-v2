...
const InputField = React.forwardRef<Input, InputFieldProps>((props, ref) => {
  ...
  const validationState = {
    status: autoStatus && error && isTouched ? 'danger' : undefined, // Set status to danger if there's an error and field is touched
    ...
    <Controller
      ...
      render={({field: {onChange, value}}) => (
        <Input
          ...
          ref={ref}
          {...validationState} // Pass the validation state (status and caption)
        />
      )}
    />
  );
